...
--V. MOST COMMON DISEASE (MCD) FOR EACH SPECIES BASED ON TOTAL CASES OVER THE LAST 5 YEARS
		
	-- FIRST, PUT TOTAL CASES AND TOTAL DEATHS BY SPECIES AND DISEASE IN A VIEW
	-- USE CASE STMNT AND LAG() TO ADD AN EXTRA COLUMN THAT INDICATES THE MCD FOR EACH DISEASE
	
	CREATE VIEW MOST_COMMON_DISEASE AS
	WITH MCD_CTE AS (
		SELECT SPECIES, DISEASE, SUM(NEW_CASES) AS TOTAL_CASES, 
		SUM(DEATHS) AS TOTAL_DEATHS, (SUM(DEATHS)/SUM(NEW_CASES)*100) AS CFR
		FROM ANIMAL_HEALTH_REPORT
		WHERE (YEAR >= 2016) AND (NEW_CASES <> 0) AND (SPECIES <> '-')  -- SOME SPECIES ARE REPORTED AS '-'
		GROUP BY SPECIES, DISEASE )										-- WE DON'T NEED THEM
	SELECT *,
	CASE
		WHEN (LAG(SPECIES, 1) OVER (PARTITION BY SPECIES ORDER BY SPECIES, TOTAL_CASES DESC ) =	 --SKIP THE SPECIES WHEN THE
		LAG(SPECIES, 1) OVER (PARTITION BY SPECIES ORDER BY SPECIES, TOTAL_CASES DESC )) THEN '' --PREVIOUS SPECIES IS THE SAME
		ELSE DISEASE														   
	END AS MCD
	FROM MCD_CTE;

	-- THEN, FECTH FOR MOST COMMON DISEASES FROM THE VIEW

	SELECT SPECIES, MCD, TOTAL_CASES, TOTAL_DEATHS, CFR FROM MOST_COMMON_DISEASE
	WHERE MCD <> ' ' 
